...
Назначение: повторяемые dev/run/build/test/deploy сбои и проверенные способы диагностики.

Источник истины по текущему поведению: код, Docker runtime, package scripts и профильные SoR-доки.
Production-first troubleshooting хранится отдельно в [deploy/README.md](../../deploy/README.md).
...
- **Симптом:** `pnpm dev:web` выводит `Next.js ... (Turbopack)` и `Starting...`, порт `3001` слушается, но `/login` не отвечает.
- **Команда:** `pnpm dev:web`
- **Причина:** Turbopack dev path зависает до readiness на текущем Next.js/Web контуре.
- **Фикс:** использовать webpack dev server: `pnpm --filter web exec next dev --webpack` или штатный `pnpm dev:web`, где webpack-режим закреплён в `apps/web/package.json`.
- **Проверка:** в выводе есть `Next.js ... (webpack)` и `Ready`, а `curl -s -I http://localhost:3001/login` возвращает `HTTP/1.1 200 OK`.
